...
# Mode-driven objective API. With none of these set, behavior is identical
# to plain deathmatch. `target` remains for opportunistic stomping while a
# positional goal drives navigation.
var move_goal: Vector2 = Vector2.ZERO
var move_goal_active: bool = false
var hold_at_goal: bool = false            # stand still on arrival (KOTH)
var goal_arrive_radius: float = 40.0      # > WALK_REACH_EPS so friction settles inside it
var priority_target: CharacterController = null

func set_move_goal(pos: Vector2, hold: bool = false) -> void:
	move_goal = pos
	move_goal_active = true
	hold_at_goal = hold

func clear_move_goal() -> void:
	move_goal_active = false
	hold_at_goal = false

func set_priority_target(c: CharacterController) -> void:
	priority_target = c
...
func _update_strategy() -> void:
	_refresh_target()
	if target == null and not move_goal_active:
		current_plan.clear()
		_clear_active_edge()
		return

	var start_node := _resolve_node_for_body(character)
	var target_node: LevelNavigation.NodeEntry
	if move_goal_active:
		target_node = navigation.find_closest_node(move_goal)
	else:
		target_node = _resolve_node_for_body(target)
...
func _refresh_target() -> void:
	# Mode-pinned target (e.g. everyone hunts the chicken). A despawned
	# priority target falls through to normal nearest-target picking.
	if priority_target != null and is_instance_valid(priority_target) \
			and not priority_target.is_despawned and priority_target != character:
...
## Clears the current plan AND the replan memo, so the next think tick
## rebuilds the path. Clearing only the plan leaves the memo claiming the
## path is current -- fatal against static goals (coin/zone) that never
## change platform and thus never re-trigger a rebuild.
func _invalidate_plan() -> void:
	current_plan.clear()
	current_start_node_id = -1
	current_target_platform_id = -1
	_clear_active_edge()
...
func _run_tactics() -> Dictionary:
	...
	# Parked at a hold-goal (KOTH): stand still until pushed off or retasked.
	if move_goal_active and hold_at_goal and _at_move_goal():
		_invalidate_plan()
...
func _chase_target_directly() -> Dictionary:
	var dx: float
	if move_goal_active:
		dx = move_goal.x - character.global_position.x
	elif target != null:
		dx = target.global_position.x - character.global_position.x
	else:
		return _action(0.0, false, false)
...
func _at_move_goal() -> bool:
	return absf(character.global_position.x - move_goal.x) <= goal_arrive_radius \
		and absf(character.get_foot_position().y - move_goal.y) <= 48.0 \
...
func _attempt_direct_stomp() -> Dictionary:
	...
	if rel.y < 0.0:
		return {}
	# While pursuing a positional objective, only point-blank stomps may
	# interrupt -- the full engagement range would starve the objective
	# whenever characters cluster.
	var engage_range := STOMP_HORIZONTAL_RANGE * 0.5 if move_goal_active else STOMP_HORIZONTAL_RANGE
	if absf(rel.x) > engage_range:
		return {}
