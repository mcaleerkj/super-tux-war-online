...
## Mode-driven tint (e.g. the chicken). Composes with the CPU color on the
## sprite's modulate; the spawn flash animates root modulate independently,
## so the two never conflict.
func set_tint_overlay(color: Color) -> void:
	_tint_overlay = color
	_apply_sprite_tint()

func clear_tint_overlay() -> void:
	_tint_overlay = Color.WHITE
	_apply_sprite_tint()

func _apply_sprite_tint() -> void:
	if animated_sprite:
		animated_sprite.modulate = character.get("character_color") * _tint_overlay
...
func _update_skid_state(
	...
	if _skid_active:
		...
		if not is_on_floor or not has_input or velocity_dir == _skid_target_direction or velocity_dir == 0:
			_skid_active = false
			_stop_skid_smoke()
			EventBus.skid_ended.emit(character)
		else:
	...
	if _should_start_skid(is_on_floor, velocity_x, input_direction, effective_max_speed, previous_velocity_x):
		...
		_play_skid_animation(_skid_target_direction)
		EventBus.skid_started.emit(character)
		return true
