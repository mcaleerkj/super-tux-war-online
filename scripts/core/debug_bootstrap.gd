...
func _apply_match_settings() -> void:
	...
	if _config.has_section_key(SECTION_MATCH, "mode"):
		var mode_id := str(_config.get_value(SECTION_MATCH, "mode", "frag")).strip_edges().to_lower()
		GameSettings.set_selected_mode(StringName(mode_id))
	if _config.has_section_key(SECTION_MATCH, "goal"):
		var goal := int(_config.get_value(SECTION_MATCH, "goal", 0))
		GameSettings.set_goal_for_mode(GameSettings.get_selected_mode(), goal)
	# Legacy alias: kills_to_win maps to frag's goal.
	if _config.has_section_key(SECTION_MATCH, "kills_to_win"):
		var kills := int(_config.get_value(SECTION_MATCH, "kills_to_win", 10))
		GameSettings.set_goal_for_mode(&"frag", kills)
...
func _has_npc_characters() -> bool:
	...
	for node in npcs:
		if node is CharacterController and node.is_cpu_controlled():
			return true
	return false
