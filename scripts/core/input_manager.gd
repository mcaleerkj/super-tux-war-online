...
## Returns the complete gameplay input for one physics tick. Gameplay code
## consumes this value instead of reading the global Input singleton directly,
## which lets local players, CPUs, and network peers share the same physics.
func capture_gameplay_input() -> Dictionary:
	return {
		"move_x": clampf(get_move_axis_x(), -1.0, 1.0),
		"run": Input.is_action_pressed("run"),
		"down": is_move_down_pressed(),
		"jump_pressed": is_jump_just_pressed(),
		"jump_released": is_jump_just_released(),
	}
...
func _ensure_default_actions() -> void:
	...
	_add_key_if_missing("toggle_mute", KEY_M)
